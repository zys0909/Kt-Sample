...
package com.group.dev.ui.qmhome.bottom;
...
import com.group.dev.ui.qmhome.UIViewOffsetHelper;
import com.group.dev.ui.qmhome.tools.Helper;
import com.group.dev.ui.qmhome.tools.INestedBottomView;


public abstract class NestedBottomDelegateLayout extends FrameLayout implements
        NestedScrollingChild2, NestedScrollingParent2, INestedBottomView {
    public static final String KEY_SCROLL_INFO_OFFSET = "scroll_info_bottom_dl_offset";
    ...
    private INestedBottomView.OnScrollNotifier mOnScrollNotifier;
    ...
    public NestedBottomDelegateLayout(Context context) {
        this(context, null);
    }

    public NestedBottomDelegateLayout(Context context, AttributeSet attrs) {
        this(context, null, 0);
    }

    public NestedBottomDelegateLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void init() {
        ...
        if (!(mContentView instanceof INestedBottomView)) {
            throw new IllegalStateException("the view create by onCreateContentView() " +
                    "should implement from INestedBottomView");
        }
    ...
    private int getMiniOffset() {
        INestedBottomView b = (INestedBottomView) mContentView;
        ...
        if (contentHeight != INestedBottomView.HEIGHT_IS_ENOUGH_TO_SCROLL) {
    ...
    public int getContentHeight() {
        ...
        if (bc == INestedBottomView.HEIGHT_IS_ENOUGH_TO_SCROLL || bc > mContentView.getHeight()) {
            return INestedBottomView.HEIGHT_IS_ENOUGH_TO_SCROLL;
        }
    ...
    public void checkLayout() {
        ...
        INestedBottomView bottomView = (INestedBottomView) mContentView;
    ...
    private int offsetBy(int dyUnConsumed) {
        ...
        mOnScrollNotifier.notify(-mHeaderViewOffsetHelper.getTopAndBottomOffset(),
                mHeaderView.getHeight() + ((INestedBottomView) mContentView).getScrollOffsetRange());
    ...
    public void consumeScroll(int dy) {
        if (dy == Integer.MAX_VALUE) {
            offsetBy(dy);
            ((INestedBottomView) mContentView).consumeScroll(Integer.MAX_VALUE);
            return;
        } else if (dy == Integer.MIN_VALUE) {
            ((INestedBottomView) mContentView).consumeScroll(Integer.MIN_VALUE);
            offsetBy(dy);
            return;
        }
        ((INestedBottomView) mContentView).consumeScroll(dy);
    }

    @Override
    public void smoothScrollYBy(int dy, int duration) {
        ((INestedBottomView) mContentView).smoothScrollYBy(dy, duration);
    }

    @Override
    public void stopScroll() {
        ((INestedBottomView) mContentView).stopScroll();
    }

    @Override
    public int getCurrentScroll() {
        return -mHeaderViewOffsetHelper.getTopAndBottomOffset() +
                ((INestedBottomView) mContentView).getCurrentScroll();
    }
    ...
    public int getScrollOffsetRange() {
        ...
        return mHeaderView.getHeight() - getHeaderStickyHeight() +
                ((INestedBottomView) mContentView).getScrollOffsetRange();
    }
    ...
    public void injectScrollNotifier(final OnScrollNotifier notifier) {
        ...
        if (mContentView instanceof INestedBottomView) {
            ((INestedBottomView) mContentView).injectScrollNotifier(new OnScrollNotifier() {
                @Override
    ...
    public void saveScrollInfo(@NonNull Bundle bundle) {
        ...
        if (mContentView != null) {
            ((INestedBottomView) mContentView).saveScrollInfo(bundle);
        }
    }
    ...
    private boolean isPointInHeaderBounds(int x, int y) {
        Helper.getDescendantRect(this, mHeaderView, mTempRect);
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (isBeingDragged) {
                    lastMotionY = y;
                    if (dy < 0 && ((INestedBottomView) mContentView).getCurrentScroll() > 0) {
    ...
    class ViewFlinger implements Runnable {
        ...
        Interpolator mInterpolator = Helper.QUNITIC_INTERPOLATOR;
        ...
        ViewFlinger() {
            mOverScroller = new OverScroller(getContext(), Helper.QUNITIC_INTERPOLATOR);
        }
        ...
        private void internalPostOnAnimation() {
            ...
            ViewCompat.postOnAnimation(NestedBottomDelegateLayout.this, this);
        ...
        public void fling(int velocityY) {
            ...
            if (mInterpolator != Helper.QUNITIC_INTERPOLATOR) {
                mInterpolator = Helper.QUNITIC_INTERPOLATOR;
                mOverScroller = new OverScroller(getContext(), Helper.QUNITIC_INTERPOLATOR);
            }
