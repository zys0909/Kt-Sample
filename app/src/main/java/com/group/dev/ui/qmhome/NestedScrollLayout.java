...
package com.group.dev.ui.qmhome;
...
import com.group.dev.ui.qmhome.bottom.NestedBottomAreaBehavior;
import com.group.dev.ui.qmhome.tools.Helper;
import com.group.dev.ui.qmhome.tools.INestedBottomView;
import com.group.dev.ui.qmhome.tools.INestedScrollCommon;
import com.group.dev.ui.qmhome.tools.INestedTopView;
import com.group.dev.ui.qmhome.top.NestedTopAreaBehavior;

import java.util.ArrayList;
import java.util.List;

public class NestedScrollLayout extends CoordinatorLayout implements
        NestedTopAreaBehavior.Callback, DraggableScrollBar.Callback {
    ...
    private NestedTopAreaBehavior mTopAreaBehavior;
    private NestedBottomAreaBehavior mBottomAreaBehavior;
    ...
    private DraggableScrollBar mDraggableScrollBar;
    private boolean mEnableScrollBarFadeInOut = true;
    private boolean mIsDraggableScrollBarEnabled = false;
    private int mCurrentScrollState = INestedScrollCommon.SCROLL_STATE_IDLE;
    ...
    protected DraggableScrollBar createScrollBar(Context context) {
        return new DraggableScrollBar(context);
    }
    ...
    public void setTopAreaView(View topView, @Nullable LayoutParams layoutParams) {
        if (!(topView instanceof INestedTopView)) {
            throw new IllegalStateException("topView must implement from INestedTopView");
        }
        if (mTopView != null) {
            removeView(((View) mTopView));
        }
        mTopView = (INestedTopView) topView;
        mTopView.injectScrollNotifier(new INestedScrollCommon.OnScrollNotifier() {
            @Override
        ...
        if (behavior instanceof NestedTopAreaBehavior) {
            mTopAreaBehavior = (NestedTopAreaBehavior) behavior;
        } else {
            mTopAreaBehavior = new NestedTopAreaBehavior(getContext());
    ...
    public INestedTopView getTopView() {
        return mTopView;
    }

    public INestedBottomView getBottomView() {
        return mBottomView;
    }

    public NestedTopAreaBehavior getTopAreaBehavior() {
        return mTopAreaBehavior;
    }

    public NestedBottomAreaBehavior getBottomAreaBehavior() {
        return mBottomAreaBehavior;
    }

    public void setBottomAreaView(View bottomView, @Nullable LayoutParams layoutParams) {
        if (!(bottomView instanceof INestedBottomView)) {
            throw new IllegalStateException("bottomView must implement from INestedBottomView");
        }
        if (mBottomView != null) {
            removeView(((View) mBottomView));
        }
        mBottomView = (INestedBottomView) bottomView;
        mBottomView.injectScrollNotifier(new INestedBottomView.OnScrollNotifier() {
            @Override
            public void notify(int innerOffset, int innerRange) {
        ...
        Behavior behavior = layoutParams.getBehavior();
        if (behavior instanceof NestedBottomAreaBehavior) {
            mBottomAreaBehavior = (NestedBottomAreaBehavior) behavior;
        } else {
            mBottomAreaBehavior = new NestedBottomAreaBehavior();
    ...
    public void onTopBehaviorTouchBegin() {
        dispatchScrollStateChange(
                INestedScrollCommon.SCROLL_STATE_DRAGGING, true);
    }

    @Override
    public void onTopBehaviorTouchEnd() {
        dispatchScrollStateChange(
                INestedScrollCommon.SCROLL_STATE_IDLE, true);
    }

    @Override
    public void onTopBehaviorFlingOrScrollStart() {
        dispatchScrollStateChange(
                INestedScrollCommon.SCROLL_STATE_SETTLING, true);
    }
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        if (ev.getAction() == MotionEvent.ACTION_DOWN) {
            if(mCurrentScrollState != INestedScrollCommon.SCROLL_STATE_IDLE){
    ...
    public void restoreScrollInfo(@Nullable Bundle bundle) {
        ...
        if (mTopAreaBehavior != null) {
            ...
            mTopAreaBehavior.setTopAndBottomOffset(Helper.constrain(-offset, -getOffsetRange(), 0));
        }
        if (mTopView != null) {
