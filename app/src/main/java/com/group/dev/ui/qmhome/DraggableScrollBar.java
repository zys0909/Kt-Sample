...
import com.group.common.ext.DensityUtil;
...
public class DraggableScrollBar extends View {
    ...
    private int mAdjustDistanceProtection = DensityUtil.dp2px(20);
    private int mAdjustMaxDistanceOnce = DensityUtil.dp2px( 4);
    ...
    public DraggableScrollBar(Context context, @NonNull Drawable dragDrawable) {
        this(context, (AttributeSet) null);
        mDragDrawable = dragDrawable.mutate();
    }

    public DraggableScrollBar(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    private void onDragging(Drawable drawable, float currentY) {
        ...
        percent = Helper.constrain(percent, 0f, 1f);
        if (mCallback != null) {
    ...
    public void awakenScrollBar() {
        if (mDragDrawable == null) {
            mDragDrawable = ContextCompat.getDrawable(getContext(), R.mipmap.qmui_icon_scroll_bar);
        }
