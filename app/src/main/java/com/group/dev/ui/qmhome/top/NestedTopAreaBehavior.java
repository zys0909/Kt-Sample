...
package com.group.dev.ui.qmhome.top;
...
import com.group.dev.ui.qmhome.NestedScrollLayout;
import com.group.dev.ui.qmhome.UIViewOffsetBehavior;

import static android.view.View.MEASURED_SIZE_MASK;

public class NestedTopAreaBehavior extends UIViewOffsetBehavior<View> {
    ...
    public NestedTopAreaBehavior(Context context) {
        this(context, null);
    }


    public NestedTopAreaBehavior(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public void scroll(@NonNull CoordinatorLayout parent, @NonNull View child, int dy) {
        ...
        if (child instanceof INestedTopView) {
            unConsumed = ((INestedTopView) child).consumeScroll(unConsumed);
        }
    ...
    public void onNestedPreScroll(@NonNull CoordinatorLayout parent, @NonNull View child,
        ...
        if (target == child) {
            ...
            if (dy > 0) {
                ...
                if (target instanceof INestedBottomView) {
                    int contentHeight = ((INestedBottomView) target).getContentHeight();
                    int minOffset;
        ...
        if (target == child) {
            ...
            if (dyUnconsumed > 0) {
                ...
                if (bottomView == null || bottomView.getVisibility() == View.GONE) {
                    ...
                    int contentHeight = ((INestedBottomView) bottomView).getContentHeight();
                    ...
                    if (canContentScroll) {
                        ((INestedBottomView) bottomView).consumeScroll(dyUnconsumed);
                    }
                }
            }
        } else {
            ...
            if (dyUnconsumed < 0) {
                ...
                if (child instanceof INestedTopView) {
                    ((INestedTopView) child).consumeScroll(dyUnconsumed);
                }
            }
        }
    }
    ...
    private View findBottomView(CoordinatorLayout parent) {
        for (int i = 0; i < parent.getChildCount(); i++) {
            ...
            if (child instanceof INestedBottomView) {
                return child;
            }
        }
        return null;
    ...
    class ViewFlinger implements Runnable {
        ...
        ViewFlinger(Context context) {
            mOverScroller = new OverScroller(context, Helper.QUNITIC_INTERPOLATOR);
        }
        ...
        public void run() {
            ...
            if (scroller.computeScrollOffset()) {
                ...
                if (mCurrentParent != null && mCurrentChild != null) {
                    ...
                    if (mCurrentParent instanceof NestedScrollLayout) {
                        NestedScrollLayout layout = (NestedScrollLayout) mCurrentParent;
        ...
        private void onFlingOrScrollStart(CoordinatorLayout parent, View child) {
            ...
            if (mInterpolator != Helper.QUNITIC_INTERPOLATOR) {
                ...
                mOverScroller = new OverScroller(mCurrentParent.getContext(), Helper.QUNITIC_INTERPOLATOR);
            }
        }
